"""
Combine n*n*n whitematter voxels into a single block
n is the bin size

Goal: create transition matrix of wm connections
"""
binsize = 8
...
if not os.path.exists('./ori2binned.pkl'):
    '''build mapping from original voxels to binned blocks'''
    # binned wm index starting from 0
    cur_binned_wm_idx = 0
    bin_coor2idx = {}
    ori2binned = {}
    for i in range(len(idx2voxelIJK)):
        cur_coor = idx2voxelIJK[i]
        binned_coor = (cur_coor/binsize).astype(int)
        # turn the str key back to array: np.asarray([int(word) for word in key[1:-1].split(' ')])
        if str(binned_coor) not in bin_coor2idx:
            bin_coor2idx[str(binned_coor)] = cur_binned_wm_idx
            cur_binned_wm_idx += 1
        ori2binned[i] = bin_coor2idx[str(binned_coor)]
    # number of blocks:
    print(len(set(ori2binned.values())))

    with open('ori2binned.pkl', 'wb') as f:
        pickle.dump(ori2binned, f, pickle.HIGHEST_PROTOCOL)
    print('original to binned coordinates dict saved.')
else:
    with open('ori2binned.pkl', 'rb') as f:
        ori2binned = pickle.load(f)
    print('original to binned coordinates dict loaded.')
